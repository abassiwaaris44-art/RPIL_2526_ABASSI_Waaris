...
Page web unique permettant à un mentoré de rechercher des mentors compatibles,
sans authentification. Le matching est calculé côté serveur (Django) à partir
des matières demandées et de l'heure souhaitée, avec une tolérance horaire de
± 1 heure.

## Stack

- **Frontend** : HTML / CSS / JavaScript vanilla (une seule page, appel AJAX vers l'API JSON)
...
- L'interface d'administration Django (`/admin/`) permet d'ajouter/modifier
  des mentors manuellement une fois un superutilisateur créé
  (`python manage.py createsuperuser`).
